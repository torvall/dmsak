...
# 
#     Usage: upmodule.sh [-d <path-to-drupal-dirs> [-v X] | -w <path-to-web-dir>] [-N] <path-to-module-tarball>
#
#   Example: upmodule.sh -d /var/wwwlib -v 5 -N module-5.x-1.10.tar.gz
#        Or: upmodule.sh -w /var/www/example.com -v 5 -N module-5.x-1.10.tar.gz
# Or simply: upmodule.sh module-5.x-1.10.tar.gz
#
...
if [ "$HELP_REQUESTED" = "TRUE" ]; then
	...
	echo 1>&2 ""
	echo 1>&2 "Usage: $0 [-d <path-to-drupal-dirs>] [-w <path-to-web-dir>] [-v X] [-N] <path-to-module-tarball>"
	...
	echo 1>&2 ""
	echo 1>&2 "Example: $0 -d /var/wwwlib -w /var/www -v 5 -N module-5.x-1.10.tar.gz"
...
if [ ! "$NO_BACKUP" = "TRUE" ]; then
	MODULE_BACKUP_TARBALL=$BACKUP_DIR/$MODULE_NAME`date +_%Y%m%d%H%M`_backup.tar.gz
